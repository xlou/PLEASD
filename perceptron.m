function [w, D, perceptron_meta] = perceptron(w, D, functors, settings)
% [w, D, perceptron_meta] = perceptron(w, D, functors, settings) is
% the entry function for structured perceptron.
% 
% Input:
%       w:          initial parameter
...
while eta_hat > eta && rho_hat > rho && iter < max_iter,
    ...
    for n = 1:length(D)
        ...
        % update model
        w = w + functors.joint_feature(d.x, d.y_dot) - functors.joint_feature(d.x, d.y_hat);
